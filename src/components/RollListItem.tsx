...
const RollListItem: React.FC<RollListItemProps> = ({ roll, onDelete, onAssignAlbum, isDeveloping = false, assignActionIcon = 'add' }) => {
  ...
  const filmStripBg = `url("data:image/svg+xml;utf8,${encodeURIComponent(`
    <svg xmlns='http://www.w3.org/2000/svg' width='200' height='100' viewBox='0 0 200 100'>
      <rect width='200' height='100' fill='%23282828'/>
      <!-- sprocket holes top -->
      ${Array.from({ length: 10 }).map((_,i) =>
        `<rect x='${i*20+4}' y='5' rx='2' ry='2' width='12' height='8' fill='%23cccccc'/>`
      ).join('')}
      <!-- sprocket holes bottom -->
      ${Array.from({ length: 10 }).map((_,i) =>
        `<rect x='${i*20+4}' y='87' rx='2' ry='2' width='12' height='8' fill='%23cccccc'/>`
      ).join('')}
    </svg>
  `)}")`;
  ...
  if (isDeveloping) {
    return (
      <div className="bg-neutral-800/50 rounded-xl overflow-hidden border border-neutral-700/30">
        ...
        <div className="flex items-center bg-neutral-900 pl-2">
          <FilmCanister />
          <div
            className="overflow-x-auto no-scrollbar h-28"
            style={{
