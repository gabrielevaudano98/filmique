  ...
  return (
    <div className="relative w-full h-20 flex items-center">
      ...
      <div className="absolute left-1/2 -translate-x-1/2 top-2 z-10 flex flex-col items-center pointer-events-none">
        <span className="text-brand-orange-start font-bold text-sm font-mono">{value.toString().replace('f/','')}</span>
        <div className="w-0.5 h-6 bg-brand-orange rounded-full mt-1"></div>
      </div>
      ...
      >
        ...
        <div className={`flex items-center h-full ${paddingX}`}>
          {options.map((opt, i) => (
            ...
            >
              <div className={`w-px ${getLineHeightClass(opt, type)} ${getLineMarginClass(opt)} ${opt === value ? 'bg-brand-orange-start' : 'bg-gray-500'}`}></div>
            </button>
          ))}
        </div>
      </div>
