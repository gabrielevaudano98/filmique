...
import PrivateAlbumModal from './PrivateAlbumModal';
...
const AlbumCard: React.FC<AlbumCardProps> = ({ album, onClick }) => {
  ...
  const info = visibilityInfo[album.type as 'private' | 'unlisted' | 'public'] || visibilityInfo.private;
  const Icon = info.icon;

  const [showPrivateModal, setShowPrivateModal] = useState(false);

  const handleClick = () => {
    // If the current viewer is not the album owner and album is not public, show disclaimer
    const isOwner = profile && profile.id === album.user_id;
    if (!isOwner && album.type !== 'public') {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="absolute top-3 right-3 bg-gray-900/50 backdrop-blur-sm rounded-full px-2 py-1 text-xs flex items-center gap-1.5">
