...
import { Plus, User, ArrowDown, Users, Trophy } from 'lucide-react';
...
const SocialView: React.FC = () => {
  ...
  const [feedSection, setFeedSection] = useState<'community' | 'profile' | 'challenges'>('community');
  ...
  const sectionOrder = useMemo(() => ['community', 'profile', 'challenges'], []);
  ...
  const segmentOptions = [
    ...
    { value: 'challenges', label: 'Challenges', icon: Trophy, colors: { from: 'from-accent-teal', to: 'to-emerald-500', shadow: 'shadow-emerald-500/30' }, description: 'Challenges' },
  ];
