...
import ManageRollsOnShelfModal from './ManageRollsOnShelfModal';
import { Roll } from '../types';
import RollListItem from './RollListItem';
import ConfirmDeleteModal from './ConfirmDeleteModal';

const ShelfDetailView: React.FC = () => {
  const { selectedShelf, setCurrentView, setSelectedShelf, deleteRoll, removeRollFromShelf } = useAppContext();
  ...
  const handleRemoveFromShelf = (roll: Roll) => {
    removeRollFromShelf(roll.id);
  };

  const rolls = selectedShelf.rolls || [];
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <div className="mb-8">
        <h2 className="text-3xl font-bold text-white">{selectedShelf.title}</h2>
      ...
      {rolls.length > 0 ? (
        <div className="space-y-2">
          {rolls.map(roll => (
            <RollListItem
              ...
              onAssignAlbum={handleRemoveFromShelf}
        ...
        <div className="text-center py-16 px-4 col-span-full bg-gray-800/50 rounded-lg">
          ...
          <h3 className="text-2xl font-semibold mb-2 text-white">Empty Shelf</h3>
          <p className="text-gray-400 mb-6 max-w-md mx-auto">
            This shelf doesn't have any photos yet. Add some rolls to see them here.
          </p>
        </div>
      )}

      {showManageModal && (
        <ManageRollsOnShelfModal
          shelf={selectedShelf}
