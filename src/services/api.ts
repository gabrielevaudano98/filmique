...
function withDeduplication<T extends (...args: any[]) => Promise<any>>(fn: T): T {
...
export const getProfile = withDeduplication((userId: string) => supabase.from('profiles').select('*').eq('id', userId).single());
...
// Film Stocks
export const fetchFilmStocks = withDeduplication(() => supabase.from('film_stocks').select('*'));

// Rolls & Photos
export const fetchAllRolls = withDeduplication((userId: string) => supabase.from('rolls').select('*, photos(*), albums(title)').eq('user_id', userId).order('created_at', { ascending: false }));
...
export const fetchFeedPosts = withDeduplication(() => supabase.from('posts').select(POST_SELECT_QUERY).order('created_at', { ascending: false }).limit(50));
...
// Albums
export const fetchAlbums = withDeduplication((userId: string) => supabase.from('albums').select('*, rolls(*, photos(*))').eq('user_id', userId));
