...
function App() {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white flex flex-col">
      ...
      <nav className={`fixed bottom-0 left-0 right-0 w-full bg-gray-800 border-t border-gray-700 px-2 py-1 safe-area-bottom z-50 transition-opacity duration-300 ${currentView === 'camera' ? 'opacity-0 pointer-events-none' : 'opacity-100 pointer-events-auto'}`}>
        <div className="flex items-center justify-around max-w-md mx-auto py-1">
          ...
          <button onClick={() => setCurrentView('camera')} className={`flex items-center justify-center w-14 h-14 rounded-full shadow-2xl transition-all duration-300 ease-in-out translate-y-[-12px] ring-2 ring-amber-300 ring-offset-2 ring-offset-gray-800 ${currentView === 'camera' ? 'bg-gray-700 text-gray-400 border-4 border-gray-600' : 'bg-amber-500 text-white hover:bg-amber-400 border-4 border-amber-200'}`} aria-label="Camera">
